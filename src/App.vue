...
watch(
  isCompactViewport,
  (compact) => {
    document.documentElement.classList.toggle('force-compact', compact)
  },
  { immediate: true },
)
...
onMounted(() => {
  ...
  window.addEventListener('resize', detectCompactViewport)
  window.addEventListener('orientationchange', detectCompactViewport)
  window.visualViewport?.addEventListener('resize', detectCompactViewport)
  document.addEventListener('click', closeLangDropdown)
  document.addEventListener('click', closeMobileDropdown)
})

onUnmounted(() => {
  window.removeEventListener('resize', detectCompactViewport)
  window.removeEventListener('orientationchange', detectCompactViewport)
  window.visualViewport?.removeEventListener('resize', detectCompactViewport)
  document.documentElement.classList.remove('force-compact')
...
/** Bump when the announcement content changes so users who dismissed a prior release see the new one. */
const ANNOUNCEMENT_VERSION = '2026-04-18-update4'
...
<template>
  <div class="site-shell" :class="{ 'force-mobile-nav': isCompactViewport }">
    <header class="site-header">
      ...
      <nav class="site-nav">
        <a
          ...
          style="display: inline-flex; align-items: center; gap: 4px; color: #333e49; font-weight: 600; text-decoration: none;"
        ...
        <div class="lang-dropdown" :class="{ 'is-open': isLangOpen }" ref="langDropdownRef">
          <button class="lang-dropdown-trigger" type="button" @click.stop.prevent="toggleLangDropdown" :aria-label="t('app.language.label')" :aria-expanded="isLangOpen">
            {{ currentLabel }}
